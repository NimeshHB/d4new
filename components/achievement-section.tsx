export function AchievementSection() {
    return (
        <section id="achievement" className="relative py-20 px-6 bg-white text-[color:var(--color-foreground)] overflow-hidden">
            ...
            <div className="container mx-auto">
                <div className="max-w-5xl mx-auto text-center">
                    <h2 className="mb-6 text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-extrabold uppercase tracking-tight leading-tight">
                        <span className="text-[color:var(--color-foreground)] drop-shadow-[0_8px_40px_rgba(0,223,130,0.12)]">
                            SOME NUMBER OF OUR ACHIEVEMENT
                        </span>
                    </h2>
                    <p className="max-w-3xl mx-auto text-[color:var(--color-muted-foreground)] text-sm sm:text-base md:text-lg leading-relaxed mb-12">
...
function StatCard({ icon, number, label }: { icon: React.ReactNode; number: string; label: string }) {
    return (
        <div className="relative rounded-2xl bg-white p-6 flex flex-col items-start gap-4 transition-transform hover:-translate-y-2 group shadow-sm">
            ...
            <div className="mt-2">
                <div
                    className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-[color:var(--color-foreground)] tracking-tight"
                    style={{ textShadow: '0 1px 0 rgba(0,0,0,0.04)' }}
                >
                    {number}
                </div>
                <div className="text-sm text-[color:var(--color-muted-foreground)] font-medium uppercase mt-2">{label}</div>
            </div>
